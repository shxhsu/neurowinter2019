...
# bash fp_no_b2000.sh subject
# eg to run B12: bash fp_no_b2000.sh B12

# Last edited on 20/03/2019
# runs topup + eddy
# this script creates a new dwi file without the b2000 volumes by extracting
# everything else and merging them

# TO DO:
# if you are using a VM, change the "eddy" command on line 143 to "eddy_openmp"
...
echo $(date +%r)
start=$SECONDS
eddy --imain=$extra_slice_file --mask=$mask_file --index=$index --acqp=$acq_param --bvecs=$bvec_file --bvals=$bval_file --topup="topup_Olf_Blind_"$subject"_dwi" --out=$out_file
end=$SECONDS
duration=$((end-start))
